...
distances = vl_alldist2(double(train_image_feats'), double(test_image_feats'));
k_neighbors = 15;
for indx=1:M
    [~, idx] = sort(distances(:,indx));
    my_labels = train_labels(idx(1:k_neighbors),1);
    for i=1:k_neighbors
        if sum(strcmp(my_labels{i}, my_labels)) > floor((k_neighbors+1)/2)
            predicted_categories{indx,1} = my_labels{i};
            break
        elseif i == k_neighbors
            predicted_categories{indx,1} = my_labels{1};
        end
    end  
end
end
